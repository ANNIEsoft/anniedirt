...
# removed 9/8/18 as not finding fermiapp
# "/grid/fermiapp/products/common/etc/setup: No such file or directory"
...
setup_new()
{
# same setup commands as used for WCSim, to pull the new root6, geant4.10, and genie 2.12.0
export CODE_BASE=/cvmfs/fermilab.opensciencegrid.org/products
source ${CODE_BASE}/common/etc/setup
export PRODUCTS=${PRODUCTS}:${CODE_BASE}/larsoft
...
setup -q debug:e9 clhep v2_2_0_8  ## 9-8-18, BOTH clhep libs not found causing errors???? "couldn't find libCLHEP-2.2.0.5.so"
...
# XXX replaced setup script with a new one that loads genie 2.12, geant4.10 and root 6
# XXX instead of genie 2.8.6, geant4.9.6 and root 5.4
...
# XXX executable file in this folder has been replaced with the new root6+genie2.12+geant4.10 version
# XXX old executable is now called g4annie_dirt_flux_genie2.8.6robs
...
if [ ! -f $GDMLBASE ]; then
  ...
  if [ ! -f $GDMLBASE ]; then
    ...
    if [ ${RWHNODE} -eq 0 ]; then
      ...
      # XXX VVV modified to point to new ROOT 6, Geant 4.10 build of annie_g4dirt
      for GEOMLOC in /annie/app/users/moflaher/anniedirt/tank_flux_root6_genie212/G4ANNIE/src \
